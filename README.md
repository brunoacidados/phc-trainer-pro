...
- **Command Palette (Ctrl/⌘+K)**: pesquisa global de missões, dicionário, enciclopédia e circuitos + navegação rápida
- **Página 📊 Progresso**: heatmap de atividade (18 semanas), gráficos por nível, grelha de 24 conquistas e portefólio de provas (export CSV)
- **IA em streaming (SSE)**: as respostas do Professor aparecem token-a-token; **histórico de chat persistente** por conta
- **RAG semântico (P4)**: busca vetorial da Enciclopédia/esquema/missões (embeddings Gemini) injetada no prompt — mais fundamentado, menos "alucinações"; fallback automático para o mini-RAG por palavras-chave
